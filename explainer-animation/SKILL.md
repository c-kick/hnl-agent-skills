...
- **Language:** English by default. If the user names a language, **all** speech and
  **all** on-screen text use that language. There is no mixed-language narration:
  ...
  - spell brand names and acronyms phonetically *for that voice* (e.g. "H N L trader",
    "R.S.I."), and keep code identifiers off-screen or treat them as proper nouns;
...
All scripts are in `scripts/` next to this file. Work in the session scratchpad (`$W`),
copy the scripts there, and publish only the deliverables into the project.

1. **Toolchain** (no sudo): `scripts/setup.sh $W`, then
...
3. **Narration:** `VOICE=… RATE=-4% node tts.mjs lines.json` (Edge neural voices; the
   ...
   Then `node assemble.mjs` → `narration.mp3`. Tune `START`/`GAP` env vars so the total is ≤ 60 s.
   If a higher-quality TTS API key is present in the environment or the project `.env`
   (e.g. ElevenLabs or OpenAI), you may use it instead, but you then need word timings from
   another source (per-line clips + proportional estimates are fine).
4. **Fonts:** `scripts/fonts.sh "Caveat:wght@700" "Permanent+Marker" "Special+Elite" "Abril+Fatface" "Patrick+Hand"`
   (swap in families that cover your language).
5. **Animation source** (`src.html`): start from `reference/engine-example.html`. Keep the
   engine and rewrite the scenes, timings and sound events for the new subject. Reusable parts:
...
6. **Build + QA loop:** `OUTDIR=<project>/demos/explainer node build.mjs`, then
   ...
   `./sheet.sh X.jpg t1 t2 t3 t4`. Check every scene at least once, mid-animation and settled.
   `node live.mjs` smoke-tests real playback, seek and the scheduler in the browser.
...
8. **Export:** `NOAUDIO=1 node render.mjs <name>.mp4` (in the background): 1920×1080, 30 fps,
   x264 CRF 17 `-tune animation`, AAC 192k, faststart. Verify with ffprobe (duration, streams)
   and one contact sheet of frames pulled from the MP4 itself.
...
- **Sync on the word:** every visual beat keys off a timestamp in `cues.json` (a check lands
  on "cash?", a stamp on "Only then", a freeze on "stops"). This does the most for perceived quality.
...
- Use handwritten red annotations ("real prices ↖") to label concepts instead of subtitles.
...
- Use one dramatic audio gesture (e.g. a tape-stop to silence on "halt/stop") to punctuate
  the key safety or feature moment.
...
- The MP4 and HTML both play start to finish with no console errors. Duration is 30–60 s, the
  audio is −16 LUFS, the voice is clearly on top, and every scene has passed a contact-sheet check.
